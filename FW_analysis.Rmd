...
library(reshape2)
theme_set(theme_bw())
df=read.csv("FW_946.csv")
library(stringr)
df$age <- as.numeric(str_replace(as.character(df$word_age)," months",""))
df$currage <- as.numeric(str_replace(as.character(df$current_age), " year", ""))
...
freqs$word_type <- factor(freqs$word_type, 
                                  ...
                                  decreasing = TRUE)])))
qplot(word_type, prop, fill=gender, geom="bar", stat="identity",
      position="dodge", 
  ...
  theme(axis.text.x=element_text(angle=90, hjust=1, vjust=.5)) + facet_wrap(~age.grp)
